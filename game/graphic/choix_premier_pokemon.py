                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    for i, pokemon in enumerate(self.pokemon_starter):
                        ...
                        if image_rect.collidepoint(mouse_pos):
                            ...
                            try:
                                with open(os.path.join(self.script_dir, 'equipe.json'), 'r') as f:
                                    equipe = json.load(f)
                            ...
                            with open(os.path.join(self.script_dir, 'equipe.json'), 'w') as f:
                                json.dump(equipe, f, indent=4, ensure_ascii=False)
                            with open(os.path.join(self.script_dir, 'pokedex.json'), 'r') as f:
                                pokedex_data = json.load(f)
                            for p in pokedex_data:
                                if int(p['id']) == pokemon.id:
                                    p['hidden'] = False
                                    break
                            with open(os.path.join(self.script_dir, 'pokedex.json'), 'w') as f:
